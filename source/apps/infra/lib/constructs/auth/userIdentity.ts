...
export interface UserIdentityProps {
  ...
  /** Initial admin email. Only used when creating an owned UserPool (ignored for external). */
  adminEmail?: string;
  namespace: string;
  /**
   * When provided, an existing Cognito UserPool is imported rather than created.
   * The external pool MUST satisfy the requirements defined in
   * docs/external-userpool-contract.md — groups, custom attributes, and triggers
   * must be pre-configured on the pool before deploying this stack.
   */
  externalUserPoolId?: string;
  /**
   * App Client ID from the external UserPool.
   * Required when externalUserPoolId is set. The client must allow at minimum
   * the USER_SRP_AUTH flow and have no client secret (browser-based SPA client).
   */
  externalUserPoolClientId?: string;
...
export class UserIdentity extends Construct {
  ...
  /** IUserPoolClient — concrete UserPoolClient when owned, imported reference when external. */
  ...
  /** Undefined when using an external UserPool (email templates are the pool owner's responsibility). */
  readonly emailTemplateSyncHandler?: NodeLambdaFunction;

  /** Undefined when using an external UserPool (trigger lambdas are not deployed). */
  readonly preSignUpErrorAlarm?: Alarm;
  /** Undefined when using an external UserPool (trigger lambdas are not deployed). */
  readonly postSignUpErrorAlarm?: Alarm;
  readonly sesAlarms: CfnAlarm[];

  /** True when this construct imported an external pool rather than creating one. */
  readonly isExternalUserPool: boolean;
  ...
  constructor(scope: Construct, id: string, props: UserIdentityProps) {
    ...
    const { dynamoDBTable, globalSettings, adminEmail, namespace, externalUserPoolId, externalUserPoolClientId } =
      props;
    this.namespace = namespace;
    this.sesAlarms = [];
    this.isExternalUserPool = !!externalUserPoolId;

    if (externalUserPoolId) {
      // ── External UserPool path ───────────────────────────────────────────
      // Import the pool by ID. All trigger lambdas, groups, custom attributes,
      // and email configuration are the external pool owner's responsibility.
      // See docs/external-userpool-contract.md for what the external pool must provide.

      if (!externalUserPoolClientId) {
        throw new Error(
          'UserIdentity: externalUserPoolClientId is required when externalUserPoolId is set. ' +
            'Provide the App Client ID from the external UserPool.',
        );
      }

      this.userPool = UserPool.fromUserPoolId(this, 'UserPool', externalUserPoolId);
      ...
      // ── Owned UserPool path ──────────────────────────────────────────────
      // Full provisioning: triggers, groups, email config, admin bootstrap.

      // Configure the pre-signup hook
      const preSignUpFn = new NodeLambdaFunction(this, 'PreSignUpFunction', {
        entry: path.join(
          __dirname,
          '../../../../../libs/lambda/src/cognito/handlers/preSignUp.ts',
        ),
        ...
        environment: {
          ...
          AWS_APPCONFIG_CONFIGURATION_PROFILE_ID:
            globalSettings.configurationProfile.attrConfigurationProfileId,
      ...
      const postConfirmationFn = new NodeLambdaFunction(this, 'PostConfirmationFunction', {
        entry: path.join(
          __dirname,
          '../../../../../libs/lambda/src/cognito/handlers/postConfirmation.ts',
        ),
      ...
      ownedPool.applyRemovalPolicy(RemovalPolicy.DESTROY); // TODO: link to config value
      this.userPool = ownedPool;
      ...
      const cognitoEmailMetricFn = new NodeLambdaFunction(this, 'CognitoEmailMetricFunction', {
        entry: path.join(
          __dirname,
          '../../../../../libs/lambda/src/cognito/handlers/cognitoEmailMetric.ts',
        ),
      ...
      const cfnUserPool = ownedPool.node.defaultChild as CfnUserPool;
      cfnUserPool.addPropertyOverride(
        'LambdaConfig.CustomMessage',
        cognitoEmailMetricFn.functionArn,
      );
      ...
      cognitoEmailMetricFn.addPermission('AllowCognitoInvoke', {
        ...
        sourceArn: ownedPool.userPoolArn,
      });

      const sesPropsProvided =
        props.isSesEnabled !== undefined && props.sesVerifiedEmail !== undefined;
      if (sesPropsProvided && props.isSesEnabled && props.sesVerifiedEmail) {
        ...
        cfnUserPool.emailConfiguration = {
          ...
          sourceArn: Token.asString(
            Fn.conditionIf(props.isSesEnabled.logicalId, sesIdentityArn, Aws.NO_VALUE),
          ),
          from: Token.asString(
            Fn.conditionIf(props.isSesEnabled.logicalId, props.sesVerifiedEmail, Aws.NO_VALUE),
          ),
        };
      ...
      const ownedClient = ownedPool.addClient('WebClient', {
      ...
      ownedClient.applyRemovalPolicy(RemovalPolicy.DESTROY); // TODO: link to config value
      this.userPoolClient = ownedClient;
      ...
      if (adminEmail) {
        const addAdminToGroupFn = new NodeLambdaFunction(this, 'AddAdminToGroupFunction', {
          entry: path.join(
            __dirname,
            '../../../../../libs/lambda/src/cognito/handlers/addAdminToGroup.ts',
          ),
        ...
        addAdminToGroupFn.addToRolePolicy(
          new PolicyStatement({
            ...
            resources: [ownedPool.userPoolArn],
          }),
        );
        ...
        new CustomResource(this, 'AddAdminToGroup', {
          ...
          properties: {
            userPoolId: ownedPool.userPoolId,
      ...
      this.emailTemplateSyncHandler = new NodeLambdaFunction(this, 'EmailTemplateSyncHandler', {
        entry: path.join(
          __dirname,
          '../../../../../libs/lambda/src/cognito/handlers/emailTemplateSyncHandler.ts',
        ),
        functionName: `${functionNamePrefix}-EmailTemplateSyncHandler`,
        logGroupCategory: LogGroupCategory.USER_IDENTITY,
        namespace: props.namespace,
        environment: {
          USER_POOL_ID: ownedPool.userPoolId,
      ...
      this.emailTemplateSyncHandler.addToRolePolicy(
        new PolicyStatement({
          ...
          resources: [
            ownedPool.userPoolArn,
            `arn:${Stack.of(this).partition}:cloudformation:${Stack.of(this).region}:${Stack.of(this).account}:stack/${Stack.of(this).stackName}/*`,
          ],
        }),
      );
    }
    // ── Shared resources (both owned and external paths) ──────────────────

    // Configure the identity pool — always owned by this stack
    ...
    // Reacts via CloudTrail events — works for both owned and external pools.
    this.profileRoleChangeHandler = new NodeLambdaFunction(this, 'ProfileRoleChangeHandler', {
      entry: path.join(
        __dirname,
        '../../../../../libs/lambda/src/cognito/handlers/profileRoleChangeHandler.ts',
      ),
    ...
    this.profileEmailChangeHandler = new NodeLambdaFunction(this, 'ProfileEmailChangeHandler', {
      entry: path.join(
        __dirname,
        '../../../../../libs/lambda/src/cognito/handlers/profileEmailChangeHandler.ts',
      ),
   ...
   * No-op when using an external UserPool (the pool owner manages email templates).
   * Call this after the website is deployed.
   */
  updateEmailTemplateWithWebsiteUrl(websiteUrl: string) {
    if (!this.emailTemplateSyncHandler) {
      // External UserPool — email template management is the pool owner's responsibility.
      return;
    }
